...
class Calatorie:
    ...
    def __init__(
        ...
        total_timp_petrecut,
        total_timp_secunde,
        timp_petrecut_in_minute,
        timp_petrecut_in_ore,
        timp_petrecut_in_zile,
        cea_mai_scurta_cursa,
        cea_mai_lunga_cursa,
        curse_an,
    ):
    ...
    def trip_cost(self):
        cost_ron = list(
            cost["Fare Amount"] for cost in elemente if cost["Fare Currency"] == "RON"
        )
        cost_euro = list(
            cost["Fare Amount"] for cost in elemente if cost["Fare Currency"] == "EUR"
        )
    ...
    def trip_canceled(self):
        curse_anulate = Counter(
            curse["Trip or Order Status"]
            for curse in elemente
            if curse["Trip or Order Status"] == "CANCELED"
        )
        curse_anulate = curse_anulate["CANCELED"]
        return curse_anulate

    def trip_completed(self):
        curse_complete = Counter(
            curse["Trip or Order Status"]
            for curse in elemente
            if curse["Trip or Order Status"] == "COMPLETED"
        )
    ...
    def product_type(self):
        curse_produs = list(
            Counter(curse["Product Type"] for curse in elemente).items()
        )
        return curse_produs
    ...
    def begin_trip_time(self):
        ...
        for timp in inceput_cursa:
            ...
            timp_inceput_cursa.append(
                {dt.year: timestamp, "seconds": timestamp_in_seconds}
            )
    ...
    def time_spent_trips(self):
        ...
        timp_petrecut_calatorii = [
            val1 - val2 for val1, val2 in zip(timp_sfarsit, timp_inceput)
        ]
